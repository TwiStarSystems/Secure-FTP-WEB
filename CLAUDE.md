...
**Access guard:** `config.php` runs a central guard at load time. Any `.php` entry point not listed in the `PUBLIC_ENTRYPOINTS` constant requires an authenticated session (`$_SESSION['user_id']` or `access_code_id`), otherwise redirects to `login.php`. When adding a new publicly reachable page, add it to `PUBLIC_ENTRYPOINTS`.
